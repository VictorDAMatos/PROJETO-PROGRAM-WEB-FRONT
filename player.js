...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const followArtistBtn = document.getElementById('follow-artist-btn'); // <-- Botão Seguir Adicionado
    ...
    // Listener de clique global para as listas de músicas em qualquer página
    ...
    // --- NOVA FUNÇÃO PARA ATUALIZAR BOTÃO SEGUIR ---
    function updateFollowArtistButtonState() {
        if (!followArtistBtn) return;
        const currentSong = songDatabase[currentSongIndex];
        if (isArtistFollowed(currentSong.artist)) {
            followArtistBtn.classList.add('following'); // Para estilização opcional
            followArtistBtn.querySelector('i').classList.replace('fa-user-plus', 'fa-user-check');
        } else {
            followArtistBtn.classList.remove('following');
            followArtistBtn.querySelector('i').classList.replace('fa-user-check', 'fa-user-plus');
        }
    }
    ...
    function loadSong(song) {
        if (song) {
            ...
            updateFollowArtistButtonState(); // <-- Chamada para a nova função
        }
    }
    ...
    function playSong() {
        ...
        if (document.getElementById('recentes-grid') && typeof renderRecentSongs === 'function') {
             renderRecentSongs();
        }
    }
    ...
    if (playPauseBtn) {
        ...
        likeBtn.addEventListener('click', () => {
            ...
            updateLikeButtonState();
            if (typeof renderLikedSongs === 'function') {
                renderLikedSongs();
            }
        });

        // --- NOVO EVENT LISTENER PARA O BOTÃO SEGUIR ---
        followArtistBtn.addEventListener('click', () => {
            const currentSong = songDatabase[currentSongIndex];
            toggleFollowArtist(currentSong.artist);
            updateFollowArtistButtonState();
        });
